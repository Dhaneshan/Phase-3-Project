...
public class EProductDao {
	...
	public List<EProductEntity> getAllProducts() {
		...
		return jdbcTemplate.query("select * from ecommerce_product", new RowMapper<EProductEntity>() {
	...
	public EProductEntity getProductById(long id) {
		...
		return jdbcTemplate.query("select * from ecommerce_product where id=" + id, new ResultSetExtractor<EProductEntity>() {
	...
	public void updateProduct(EProductEntity obj) {
		if (obj.getId() > 0) {
			String sql = "update ecommerce_product set name=?,price=? where id=?";
			jdbcTemplate.update(sql, obj.getName(), obj.getPrice(), obj.getId());
		} else {
			String sql = "insert into ecommerce_product (name,price) values (?,?)";
